...
void fire(Board* p1board, Board* p2board){
	...
	while(tmp != 1){
        ...
        if(p2board->board[r][c].shipType != -1 && p2board->board[r][c].wasHit == false && p2board->board[r][c].shipType == 6){
          p2board->shotsFierd = newShot(p2board->shotsFierd,shot,true);
          p2board-> hp-- ;
          p2board->board[r][c].shipType = true;
          if(p2board->hp == 0){
    ...
    if(turn == 2){
        ...
        if(p2board->board[r][c].shipType != -1 && p1board->board[r][c].shipType != 100 && p1board->board[r][c].hasShip == true ){
            p2board->shotsFierd = newShot(p2board->shotsFierd,shot,true);
            p1board-> hp-- ;
            p1board->board[r][c].shipType = true;
            if(p1board->hp == 0){
